...
- **矩形レンジ選択**: マウスドラッグ、Shift+クリック、Shift+矢印、
  Cmd+矢印で端まで、Cmd+A。行番号クリックで行選択、ヘッダクリックで
  列選択
...
- **ソート** (列ヘッダを右クリック): 昇順/降順、複数列選択時はマルチキー、
  数値と文字列はペアごとに自動判別。ヘッダに標準のソートインジケータ表示
...
- **ヘッダ名編集** (列ヘッダをダブルクリック、または右クリック →
  Rename Column) と**列幅 auto-fit** (仕切り線ダブルクリック、または
  右クリック → Auto-fit Width。20k 行超はサンプリング測定)
- **数値列の自動右寄せ** (表示のみ — 値は文字列のまま)
- **ステータスバー**: エンコーディング、デリミタ (保存時に変換)、
  改行コード、ヘッダ行トグル (先頭行の昇格/降格) を表示・変更でき、
  すべて undo 可能。行数×列数の表示付き
- **Open Recent**、ウィンドウへの **CSV/TSV ドラッグ&ドロップ**での
  オープン、**ウィンドウ位置・サイズの記憶** (複数ウィンドウはカスケード)
- **UI ローカライズ (日本語 / 英語)** — システム言語に追従。
  システム設定 › 一般 › 言語と地域でアプリごとの切替も可能
- 最大ファイルサイズ: 500 MB (超過は明確なエラーで拒否)

Phase 2 (csv-editor 機能 parity) は完了 — 残りはリリースエンジニアリング
(アイコン、パッケージング、移行案内) です。
