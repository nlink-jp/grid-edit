...
# §Code Signing → GUI apps). Pure AppKit/SwiftUI needs no JIT entitlements —
# Hardened Runtime alone suffices. Reading/writing user-selected CSV files
# requires no entitlement in a non-sandboxed app.
...
# is $(APP_NAME).app.
BREW_KIND      := cask
BREW_DESC      := Native macOS CSV/TSV editor with Japanese-encoding and delimiter auto-detection
BREW_NAME      := $(NAME)
...
build-app: build
	...
	@# SPM resource bundles (en/ja localizations). SwiftPM's own Bundle.module
	@# does NOT look here — see Sources/GridEditCore/ResourceBundle.swift, which
	@# is what actually finds them (v0.2.4 launch-crash fix).
